...
if Server then
    ...
    function NS2Gamerules:UpdateCustomNetworkSettings()
        ...
        if not self.nextTimeUpdateCustomNetworkSettings or self.nextTimeCustomNetworkSettings < kTime then
            if  Server.GetSendrate() ~= 20 or Server.GetTickrate() ~= 30 or Shared.GetSettingsVariable( "mr" ) ~= "26.000000" or Shared.GetSettingsVariable("interp") ~= "0.100000" then
...
--[[
        ...
        if not Shared.GetCheatsEnabled() and Server.IsDedicated() and
            ...
            if not self.gameInfo:GetRookieMode() and not playedTutorial[player:GetSteamId()] and player:GetPlayerLevel() < kRookieOnlyLevel then
                return false, 1
            elseif self.gameInfo:GetRookieMode() and player:GetPlayerLevel() >= kRookieAllowedLevel then
                return false, 2
            end
        end
 ]]--        
...
-------------------------------------------------------------
                    ...
                    if gamestate < kGameState.PreGame and self.firstPregameJoin
                            and self.firstPregameJoin + kStartGameVoteDelay < Shared.GetTime() then
...
-----------------------------------------------------------
        ...
        local endGameOnUnbalancedAmount = false
